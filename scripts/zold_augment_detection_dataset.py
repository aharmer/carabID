...
class YOLOAugmenter:
    ...
    def parse_polygon_annotation(self, annotation_line):
        """Parse a single polygon annotation line"""
    ...
    def process_image(self, image_path, label_path, image_augmentation_plan):
        ...
        if label_path.exists():
            with open(label_path, 'r') as f:
                for line in f:
                    if line.strip():
                        class_id, coords = self.parse_polygon_annotation(line)
    ...
    def augment_dataset(self):
        ...
        print(f"  - Input format: Polygon annotations")
        ...
        format_msg = "converted from polygon to bounding box format" if self.convert_to_bbox else "kept in polygon format"
...
def main():
    parser = argparse.ArgumentParser(description='Augment YOLO polygon dataset with random augmentation selection')
    parser.add_argument('--images-dir', required=True, help='Directory containing original images')
    parser.add_argument('--labels-dir', required=True, help='Directory containing original polygon label files')
    ...
    parser.add_argument('--convert-to-bbox', action='store_true', 
                       help='Convert polygon annotations to bounding box format (default: keep polygon format)')
...
# 1.5x dataset size with flips and rotations, convert to bbox:
# python augment_detection_dataset.py --images-dir ./original/images --labels-dir ./original/labels --output-images-dir ./augmented/images --output-labels-dir ./augmented/labels --multiplication-factor 1.5 --convert-to-bbox

# 4x dataset size with only horizontal flips:
# python augment_detection_dataset.py --images-dir ./original/images --labels-dir ./original/labels --output-images-dir ./augmented/images --output-labels-dir ./augmented/labels --no-rotation --multiplication-factor 4.0

# Custom example:
# python ./scripts/augment_detection_dataset.py --images-dir D:/Dropbox/data/carabID/imgs/combined_processed/train/images --labels-dir D:/Dropbox/data/carabID/imgs/combined_processed/train/labels --output-images-dir D:/Dropbox/data/carabID/imgs/combined_processed/train/images_aug --output-labels-dir D:/Dropbox/data/carabID/imgs/combined_processed/train/labels_aug --multiplication-factor 1 --no-flip --no-rotation --convert-to-bbox
